...
const HighlightCard = ({ name, description, price, image }: HighlightCardProps) => {
  return (
    <div className="flex-1 group relative overflow-hidden border-b md:border-b-0 md:border-r border-white/10 h-[600px] md:h-[819px]">
      ...
      <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-transparent to-transparent flex flex-col justify-end p-12">
        ...
        <div className="flex items-center justify-between opacity-0 group-hover:opacity-100 transition-opacity duration-300">
          <p className="text-white/80 max-w-[240px] font-medium">
            {description}
          </p>
          <span className="bg-primary-fixed text-primary px-4 py-2 rounded-full font-black">
            R$ {price}
          </span>
        </div>
      </div>
